...
TEST_CASE("Sigma point propagation returns expected trajectory sizes", "[propagation]") {
...

    {
        ...
        Kokkos::parallel_for("init_sigmas", num_bundles * num_sigma * 7 * num_steps, KOKKOS_LAMBDA(int idx) {
            ...
            int t = idx % num_steps;
            sigmas_combined(i, j, k, t) = 0.1 * (k + 1);  // arbitrary init
        });
        ...
        REQUIRE(host_traj.extent(2) == num_steps - 1);
        REQUIRE(host_traj.extent(3) == 7);

        // Example floating point check
        CHECK_THAT(host_traj(0, 0, 0, 0), WithinAbs(0.0, 1e2));  // just a structural test
    }
